...
#!/usr/bin/env python3
"""
run_ae.py  —  Entry point for the autoencoder-based maritime anomaly detector.

Mirrors the CLI of run.py exactly so results can be compared directly.
...
-------
  ...
  --compare            Also run the original rule-based + IF/LOF detectors and
...
# ──────────────────────────────────────────────────────────────────────────────
# Data loading  (re-uses the same dtype map as run.py)
# ──────────────────────────────────────────────────────────────────────────────
...
def main(argv=None):
    ...
    # Keep a reference to raw df for LSTM AE (before we delete it in feat building)
    raw_df_for_lstm = df.copy() if not args.no_lstm else None
    ...
    if not args.no_eval and "true_anomalous" in ae_results.columns:
        ...
        # top_detections expects score_ prefix — rename for compatibility
        ae_display = ae_results.rename(columns={
            c: c.replace("ae_score_", "score_") for c in ae_results.columns
            if c.startswith("ae_score_")
        })

...
# ──────────────────────────────────────────────────────────────
# 2. RENDEZVOUS DETECTION  (vectorized, O(N log N))
# ──────────────────────────────────────────────────────────────
def detect_rendezvous(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This is an approximation (two vessels at opposite corners of the cell
    might be 40 km apart), but it's O(N) after the groupby and runs in
    seconds on 670k rows. A true spatial join can be added later if needed.
    """
    df = df.copy()
    ...
    # 0.05° ≈ 5.5 km — tight enough to be a genuine co-location signal,
    # not just "two ships in the same ocean quadrant"
    df["_glat"] = (df["lat"] / 0.05).astype(int)
    df["_glon"] = (df["lon"] / 0.05).astype(int)

    # count distinct vessels per cell×slot — fully vectorized
    cell_counts = (
        df.groupby(["_slot", "_glat", "_glon"])["entity_id"]
          .transform("nunique")
    )
    df["rendezvous_flag"] = (cell_counts >= 2).astype(int)
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cold-start guard: for first few pings of a vessel, the rolling window is
    # empty → fill with the vessel-type median rather than the individual's
    # median (which is itself computed from all pings and would be leakage-free
    # but noisy for a 1-ping vessel).
    vtype_speed_med = df.groupby("vessel_type")["sog_knots"].transform("median")
    df["speed_roll_mean"] = (
    ...
    df["speed_roll_std"] = (
        df.groupby("entity_id")["sog_knots"]
          .transform(lambda s: s.shift(1).rolling(20, min_periods=3).std())
    )
    # fill cold-start NaNs with vessel-type population stats
    df["speed_roll_mean"] = df["speed_roll_mean"].fillna(vtype_speed_med)
    df["speed_roll_std"]  = df["speed_roll_std"].fillna(
        df.groupby("vessel_type")["sog_knots"].transform("std").fillna(1)
    )

...
def rule_based_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["rule_dark_ship"] = ((df["is_stay_int"] == 1) & (df["sog_knots"] < 0.2) & (df["stay_duration_min"] > 120))
    ...
    df["rule_illegal_fish"] = ((df["sog_knots"] < 2) & (df["dist_from_median_km"] > 20) & (df["vessel_type"] == "fishing"))
...
def evaluate(df: pd.DataFrame):
    ...
    for col in ["rule_dark_ship", "rule_spoofing", "rule_aggression", "rule_illegal_fish"]: 
        if col in df.columns: 
            prec = (df[col] & (y_true == 1)).sum() / df[col].sum() if df[col].sum() > 0 else 0

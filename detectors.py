"""
detectors.py — Anomaly detectors operating on the per-contact feature matrix.

Architecture:
  1. Rule-based detectors for the two cleanest signatures (speed teleport,
     transmission blackout) — near-perfect precision, calibrated to physics.
  2. ML detectors (Isolation Forest + LOF) for behavioural patterns that are
     only anomalous in context (slow speed near restricted areas, co-location
     far from ports, erratic close shadowing).
  3. An ensemble that takes the max score across all detectors.

No ground-truth labels are used anywhere in this module.
"""
...
def score_blackout(feat: pd.DataFrame, nominal_interval_h: float = 0.25) -> pd.Series:
    """
    Contacts that go silent for a long time and then reappear displaced.

    Score combines:
      - max silence duration (normalised against run length)
      - displacement during silence (how far the contact jumped)
    """
...
# ── 3. ML: restricted-area slow operations ────────────────────────────────────

def score_slow_in_stratum(feat: pd.DataFrame,
                           contamination: float = 0.05) -> pd.Series:
    """
    Slow, loitering behaviour inside sensitive geographic strata and far
...
# ── 5. ML: erratic close-range shadowing ─────────────────────────────────────

def score_erratic_pursuit(feat: pd.DataFrame,
                           contamination: float = 0.05) -> pd.Series:
    """
    A contact that shadows another at close range with high heading variance
    and speed bursts — the "aggressive pursuit" pattern.
...
def ensemble_scores(feat: pd.DataFrame,
    ...
    The ensemble uses a weighted maximum (rather than mean) to preserve the
    clean hard-physics signals while allowing soft ML signals to contribute.
    """
